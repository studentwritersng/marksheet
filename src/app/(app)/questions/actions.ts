...
export async function aiGenerateQuestionsMultiAction(
  ...
  const difficulty = String(formData.get("difficulty") ?? "application");
...
- Distractors must be plausible and drawn from the same topic — common misconceptions, near-miss terms, or reversed cause/effect. Never use obviously absurd options.
- For "grounded" questions, the correct option's fact must appear in the lesson note; note the reference.
...
Target difficulty: {{difficulty_level}} (recall / application / analysis)
...
Output valid JSON only, with this exact shape and no additional text before or after it:
{
  "questions": [
    ...
    : `You are an experienced Nigerian secondary school examiner setting essay questions for an exam. You will generate essay question(s) based on the lesson notes provided below, following a specific balance between lesson-note-grounded content and topic-relevant AI improvisation as specified by a grounding ratio.
...
- "extension": correct, curriculum-appropriate content on the same topic that goes beyond what the lesson note explicitly covers, but remains within the same subject, topic, and class-level scope — the kind of related knowledge a student who has mastered the lesson note would reasonably be expected to apply or extend into.
...
- Extension content should represent a natural application, deeper implication, or closely adjacent fact of the same topic — the kind of thing a well-taught student could reasonably infer or apply, not an unrelated fact that happens to share a keyword.
...
Target difficulty: {{difficulty_level}} (e.g. recall / application / analysis — pitch question phrasing and expected answer depth accordingly)

TASK
For each question:
1. Write a clear essay question testing understanding of the topic, phrased at a difficulty appropriate to difficulty_level and class_level.
2. Write a model answer that fully addresses the question, incorporating both grounded and extension content as applicable.
3. Write a rubric: a list of discrete, individually markable points, each with its own mark allocation (summing to marks_per_question), each tagged "grounded" or "extension" per the ratio rule above, and for "grounded" points, a short reference to which part of the lesson note it draws from.
...
Lesson note content:\n${combinedContent.slice(0, 8000)}`,
  ...
  if (questions.length === 0) {
    ...
    for (const q of questions) {
      ...
      const qDiff = String(qm.difficulty ?? difficulty);
