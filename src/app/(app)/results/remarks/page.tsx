...
export default async function RemarksPage(props: {
  ...
  let termResults: { studentId: string; teacherComment: string | null; principalComment: string | null }[] = [];
  ...
  const remarksMap = Object.fromEntries(termResults.map((r) => [r.studentId, { teacherComment: r.teacherComment ?? "", principalComment: r.principalComment ?? "" }]));

  return (
    <div>
      <h1 className="font-headline-lg text-headline-lg text-on-surface">Remarks</h1>
      <p className="mt-1 font-body-sm text-body-sm text-on-surface-variant">
        Write teacher and principal comments for each student. These appear on report cards.
      </p>
      <div className="mt-6">
        <RemarksForm
