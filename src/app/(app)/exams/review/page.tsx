...
export default async function ExamReviewPage() {
  ...
  if (!user.schoolId || (!canManageSchool(perms) && !canReviewExams(perms))) {
  ...
  const [exams, subjects, classes, terms, questions, classSubjects, assessmentTypes, weightings] = await Promise.all([
    prisma.exam.findMany({
      where: { schoolId: user.schoolId, status: "pending_review" },
      include: {
    ...
    <div className="flex flex-col gap-stack-lg">
      <div>
        ...
        <p className="font-body-md text-body-md text-on-surface-variant mt-1">
          Review exams submitted by teachers. Approve them so an admin can publish, or reject with feedback.
        </p>
      </div>

      {exams.length === 0 ? (
        <div className="bg-surface-container-lowest border border-outline-variant rounded-lg p-8 text-center">
          <span className="material-symbols-outlined text-5xl text-on-surface-variant mb-2">check_circle</span>
          <p className="font-body-md text-body-md text-on-surface-variant">No exams pending review. Great job!</p>
        </div>
      ) : (
        <ExamsList
          csvData={undefined}
