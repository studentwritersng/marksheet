...
export async function proprietorChangePasswordAction(
  _prev: PropChangePasswordState,
  formData: FormData,
): Promise<PropChangePasswordState> {
  const user = await getCurrentUser();
  if (!user || user.role !== "proprietor") return { error: "Not authenticated." };
  ...
  const token = createSessionToken({
    ...
    proprietorPermissionLevel: user.proprietorPermissionLevel,
  });
