  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        ...
        >{showForm ? "Cancel" : "Add Method"}</button>
      </div>
        ...
        <form action={action} className="bg-white/5 border border-white/10 rounded-xl p-5 space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <select name="type" value={methodType} onChange={e => setMethodType(e.target.value)}
            ...
            <div>
              ...
              <input name="label" required placeholder={methodType === "bank_transfer" ? "e.g. GTBank" : methodType === "cash" ? "e.g. Cash Payment" : "e.g. Paystack"}
          ...
          {methodType === "bank_transfer" && (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div>
                ...
                <input name="bankName" required placeholder="e.g. Guaranty Trust Bank"
                  className="w-full bg-white/5 border border-white/10 rounded-lg p-2 text-sm text-white placeholder:text-white/20" />
              </div>
              <div>
                <label className="text-xs text-white/50 block mb-1">Account Number</label>
                <input name="accountNumber" required placeholder="0123456789"
                  className="w-full bg-white/5 border border-white/10 rounded-lg p-2 text-sm text-white placeholder:text-white/20" />
              </div>
              <div>
                <label className="text-xs text-white/50 block mb-1">Account Name</label>
                <input name="accountName" required placeholder="e.g. Marksheet Ltd"
                  className="w-full bg-white/5 border border-white/10 rounded-lg p-2 text-sm text-white placeholder:text-white/20" />
              </div>
            </div>
          )}
          ...
          {methodType === "online" && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <input name="provider" required placeholder="e.g. Paystack"
                  className="w-full bg-white/5 border border-white/10 rounded-lg p-2 text-sm text-white placeholder:text-white/20" />
              </div>
              <div>
                <label className="text-xs text-white/50 block mb-1">Public Key</label>
                <input name="publicKey" required placeholder="pk_live_..."
                  className="w-full bg-white/5 border border-white/10 rounded-lg p-2 text-sm text-white placeholder:text-white/20" />
              </div>
            </div>
          )}
...
function MethodCard({ method }: { method: PMVM }) {
  const [togState, togAction, togPending] = useActionState(
    async () => togglePaymentMethodAction(method.id, !method.isActive), {},
  );
  const [delState, delAction, delPending] = useActionState(
    async () => deletePaymentMethodAction(method.id), {},
  );
    ...
    <div className="bg-white/[0.03] border border-white/10 rounded-xl p-4">
      <div className="flex items-center justify-between mb-2">
        <div>
          ...
          <p className="text-white/40 text-xs">{typeLabels[method.type] ?? method.type}</p>
        </div>
        {method.isActive
        ...
        <div className="text-xs text-white/50 space-y-0.5 mb-3 bg-white/[0.02] rounded-lg p-2">
          {Object.entries(method.details).map(([k, v]) => (
            <div key={k} className="flex gap-2">
              <span className="capitalize text-white/30">{k.replace(/([A-Z])/g, " $1").trim()}:</span>
      ...
      <div className="flex gap-2">
        <form action={togAction}>
          ...
          >{method.isActive ? "Deactivate" : "Activate"}</button>
        </form>
        {!method.isActive && (
